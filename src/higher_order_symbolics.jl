...
function higher_order_symbolics!(symbolic_timeseries::Vector{T}, order::Int, delays = collect(0:order-1)) where T<:Integer
    ...
    #first, remap symbols to 1:nr_symbols
    for s in 1:length(symbolic_timeseries)
        symbolic_timeseries[s] =  symbol_dictionary[symbolic_timeseries[s]]
    end

    # higher order state in vector formalism
    states = Vector{T}(undef, order)
    ...
    for i in 1+max_delay:length(symbolic_timeseries)
        ...
        for d in 1:length(states)
            states[d] = symbolic_timeseries[i-delays[d]]
        end
