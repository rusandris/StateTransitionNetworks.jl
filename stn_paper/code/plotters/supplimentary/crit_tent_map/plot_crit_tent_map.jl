...
data_dir2="../../../../data/supplimentary/crit_map_1024_1e8/"
data_dir3="../../../../data/supplimentary/crit_map_1024_1e7/"
...
#qs = data_crit_long[:,1]
#Hs = data_crit_long[:,2]
#plot!(pl_crit_Kq, qs, Hs, label=L"\tilde{K}_q(10), "*L"n=2^{10}, "*L"10^{8}", linewidth=3, alpha=1, color=:gray10)
...
qs = data_crit[:,1]
Hs = data_crit[:,2]
plot!(pl_crit_Kq, qs, Hs, label=L"\tilde{K}_q(8), "*L"n=2^{10}, "*L"10^{7}",markershape=:circle,mc=:gray10,ms=3,markerstrokewidth=0.0, linewidth=1, alpha=1, color=:gray10)
plot!(pl_crit_Kq,[1.0],[0.5],st=:scatter,mc=:red,markerstrokewidth=1,markerstrokecolor=:gray10,ms=4,label="") #theoretical value at q=1
...
Os = Int.(readdlm(data_dir*"orders_crit_static.txt"))[4:end] #don't show lower orders
qs = 0.01:0.01:2
pl_crit_Hq = plot()
for (i,o) in enumerate(Os)
...
plot!(pl_crit_Hq,[1.0],[0.5],st=:scatter,mc=:red,markerstrokewidth=1,markerstrokecolor=:gray10,ms=4,label="") #theoretical value at q=1
...
pl = plot(pl_tent_Hq,pl_tent_Kq,pl_crit_Hq,pl_crit_Kq,layout=(2,2),size=(colfig_size[1],colfig_size[2]-300),left_margin=1Plots.mm,right_margin=1Plots.mm)
