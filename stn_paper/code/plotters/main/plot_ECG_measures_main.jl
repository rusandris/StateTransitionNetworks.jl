...
samples = readdlm(data_dir*"sample_ids.txt",String)[3:4] #take last two examples
method_params = readdlm(data_dir*"method_params.txt")
grid_sizes = Int.(method_params[1,:])[end] #take only 30 bin resolution
ws = Int.(method_params[2,:])[2] #take only w=5
...
pl = plot(plots_grid...,layout = (1,length(plots_grid)),size=(colfig_size[1],colfig_size[2]-300),left_margin=7Plots.mm);
