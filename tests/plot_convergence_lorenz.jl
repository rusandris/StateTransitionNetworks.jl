...
for (i,ρ) in enumerate(params)
	...
	@show ρ
	psection = poincaresos(ds, plane, T; Ttr=Ttr, direction=+1,rootkw = (xrtol = tol, atol = tol));
	traj = psection[:,2:end] 
	...
	plot!(steps,lyaps,lw=2,lc = colors[i],xlabel="t",ylabel=L"$\Delta L/t$",xguidefontsize=18,yguidefontsize=18,tickfontsize=10,label="$ρ",legendfontsize=15,legend=:best,dpi=300)
end
